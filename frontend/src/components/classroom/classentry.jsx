...
// use async() for API calls and await for waiting for response before proceeding to next line of code
const ClassroomEntry = async () => {
  const [mode, setMode] = useState(null); // null | "join" | "create"
  const [joincode, setCode] = useState(""); // class code for joining
  const [className, setClassName] = useState(""); // class name
  const [classCode, setClassCode] = useState(""); // class code for creating
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(""); // error messages
  const [success, setSuccess] = useState(false);
  const navigate = useNavigate(); // for navigation after successful join/create

  const token_id = localStorage.getItem("access_token"); // Get the access token from localStorage

  // In a real app, you would also have state for API responses, user info, etc.
  const handleAction = () => {
  ...
  // In a real app, handleAction would make API calls to join/create classrooms and handle responses accordingly.
  const reset = () => {
  ...
  try{
    ...
    if(mode === "join" && joincode.trim()){
      // API call to join classroom with joincode
      const classjoinResponse = fetch("http://localhost:8000/online-exams/classroom/join", {
        method: "POST",
        headers: {  "Content-Type": "application/json",
                    "Authorization": `Bearer ${token_id}`, },
        body: JSON.stringify({ code: joincode }),
      });
      const joinData = await classjoinResponse.json();
      if(joinData.success){
        // Successfully joined the class
        setSuccess(true);
        navigate("/classroom"); // Navigate to the classroom page after joining

      }else{
        // Failed to join the class
        setError(joinData.message || "Failed to join the classroom.");
      }
    }else{
      // Class creation mode
      if(mode === "create" && className.trim() && classCode.trim()){
        // API call to create classroom with className and classCode
        const classcreateResponse = fetch("http://localhost:8000/online-exams/classroom/create", {
          method: "POST",
          headers: {  "Content-Type": "application/json",
                      "Authorization": `Bearer ${token_id}`, },
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .mode-arrow {
          ...
          color: rgba(99,102,241,0.6); font-size: 0.8rem;
      ...
      <div className="root">
        ...
        <div className="card">
          ...
          <div className="logo">
            ...
            <span className="logo-name">EduSpace</span>
          </div>
